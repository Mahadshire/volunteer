...
function read_all_charge($conn){
    $data = array();
    $array_data = array();
   $query ="SELECT ch.charge_id,concat(e.emp_first_name,' ',e.emp_last_name) AS Employee_Name,j.position,ch.Amount,m.month_name,ch.year,ch.description,a.bank_name,ch.user_id,ch.date FROM charge ch JOIN employee e ON ch.emp_id=e.emp_id JOIN jop_title j ON ch.title_id=j.title_id JOIN month m ON ch.month_id=m.month_id JOIN account a ON ch.account_id=a.account_id";
    $result = $conn->query($query);
