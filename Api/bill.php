...
function register_bill($conn){
    ...
    $query = "INSERT INTO bill (emp_id, month,amount,user_id,account_id,date)
     values('$employees_id', '$month', '$amount', '$user_id', '$account_id', '$date')";
...
function read_all_bill($conn){
    $data = array();
    $array_data = array();
   $query ="SELECT e.emp_first_name,e.emp_last_name,b.amount,m.month_name,a.bank_name FROM bill b JOIN employee e on b.emp_id = e.emp_id JOIN month m on b.month = m.month_id JOIN account a on b.account_id = a.account_id";
    $result = $conn->query($query);
